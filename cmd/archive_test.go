...
func TestArchiveThresholds(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:         "run should fail with exit status 104 on a threshold applied to a non existing submetric",
			noThresholds: false,
			testFilename: "testdata/thresholds/non_existing_submetric.js",
			wantErr:      true,
		},
		{

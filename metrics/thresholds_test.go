...
func TestThresholdsValidate(t *testing.T) {
	...
	t.Run("validating thresholds applied to a non-existing metric fails", func(t *testing.T) {
		...
		assert.Error(t, gotErr, "Validate should fail when applied to a non-existing metric")
		assert.ErrorIs(t, gotErr, ErrInvalidThreshold, "Validate error chain should contain an ErrInvalidThreshold error")
		assert.ErrorAs(t, gotErr, &wantErr, "Validate error chain should contain an errext.HasExitCode error")
		assert.Equal(t, exitcodes.InvalidConfig, wantErr.ExitCode(), "Validate error should have been exitcodes.InvalidConfig")
	})

	t.Run("validating thresholds applied to a non-existing submetric fails", func(t *testing.T) {
		t.Parallel()
		...
		// Collect the result of the parsing operation
		gotSingleTagErr := ts.Validate("my_metric{non:existing}", testRegistry)
		gotMultiTagErr := ts.Validate("my_metric{non:existing,really:not}", testRegistry)

		assert.Error(t, gotSingleTagErr, "Validate should fail when applied to a non-existing single tag submetric")
		assert.ErrorIs(t, gotSingleTagErr, ErrInvalidThreshold, "Validate error chain should contain an ErrInvalidThreshold error")
		assert.ErrorAs(t, gotSingleTagErr, &wantErr, "Validate error chain should contain an errext.HasExitCode error")
		assert.Equal(t, exitcodes.InvalidConfig, wantErr.ExitCode(), "Validate error should have been exitcodes.InvalidConfig")

		assert.Error(t, gotMultiTagErr, "Validate should fail when applied to a non-existing multi-tag submetric")
		assert.ErrorIs(t, gotMultiTagErr, ErrInvalidThreshold, "Validate error chain should contain an ErrInvalidThreshold error")
		assert.ErrorAs(t, gotMultiTagErr, &wantErr, "Validate error chain should contain an errext.HasExitCode error")
		assert.Equal(t, exitcodes.InvalidConfig, wantErr.ExitCode(), "Validate error should have been exitcodes.InvalidConfig")
	})
	...
	// t.Run("validating thresholds over  submetrics should work", func(t *testing.T) {
	// 	t.Parallel()

	// 	testRegistry := NewRegistry()
	// 	testMetric, err := testRegistry.NewMetric("test_counter", Counter)
	// 	require.NoError(t, err)
	// 	_, err = testMetric.AddSubmetric("foo:bar")
	// 	require.NoError(t, err)
	...
	// 	// Collect the result of the parsing operation
	// 	gotErr := ts.Validate("test_counter{foo:bar}", testRegistry)

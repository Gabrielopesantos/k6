...
func (ts *Thresholds) Validate(metricName string, r *Registry) error {
	// metricNameParserFn := func(c rune) bool {
	// 	return c == '{' || c == '}'
	// }

	// metricNameParts := strings.FieldsFunc(metricName, metricNameParserFn)
	// parsedMetricName := metricNameParts[0] // valid metric name will have at least 1 member

	parsedMetricName, parsedTags, err := ParseMetricName(metricName)
	if err != nil {
	...
	if parsedTags != nil {
		keyValues := strings.Join(parsedTags, ",")
		n, ok := metric.FindSubmetric(keyValues)
		if !ok {
			err := fmt.Errorf("%w defined on %s; reason: no submetric with tags %q found for metric %q",
				ErrInvalidThreshold, metricName, keyValues, metricName)
			return errext.WithExitCodeIfNone(err, exitcodes.InvalidConfig)
		}

		metric = metric.Submetrics[n].Metric
	}
